    ...
    return (
        ...
        >
            <div className="w-12 h-12 flex items-center justify-center mb-1">
                {food.image ? (
                    <img src={food.image} alt={food.name} className="w-full h-full object-contain" style={{ imageRendering: 'pixelated' }} />
                ) : (
                    <span className="text-2xl">🍱</span>
                )}
            </div>
            <span className="text-[10px] font-black text-slate-800 text-center leading-tight">{food.name}</span>
        </div>
    );
}
...
export default function HomePage({ foods, setFoods }) {
    ...
    const handleDeleteLog = (foodId, logIndex) => {
        if (window.confirm("確定要刪除這筆食用紀錄嗎？")) {
            ...
            const updatedFoods = foods.map(food => {
                if (food.id === foodId) {
                    const newLogs = food.logs.filter((_, index) => index !== logIndex);
                    let newStatus;
                    if (newLogs.length === 0) {
                        newStatus = 'candidate';
                    } else {
                        const hasAllergy = newLogs.some(log => log.reaction === true);
                        newStatus = hasAllergy ? 'allergy' : 'safe';
                    }
                    return { ...food, logs: newLogs, status: newStatus };
                }
                return food;
            });
    ...
    const renderArea = (id, title, bgColor, textColor, icon) => {
        let sectionFoods = filteredFoods.filter(f => {
            ...
            if (id === 'allergy') return f.status === 'allergy';
            if (id === 'candidate') {
            ...
            <div className={`mb-6 p-4 border-4 border-slate-800 rounded-2xl ${bgColor}`}>
                <div className="flex justify-between items-center mb-4">
                    <h2 className={`text-lg font-black inline-block px-4 py-1 border-2 border-slate-800 rounded-full bg-white ${textColor} flex items-center gap-2`}>
                        {icon}{title} ({sectionFoods.length})
                    </h2>
                </div>
                <SortableContext id={id} items={sectionFoods.map(f => f.id)} strategy={rectSortingStrategy}>
                    <div className="flex flex-wrap gap-2 min-h-[120px]">
                        {sectionFoods.map(food => (
                            <FoodBlock key={food.id} food={food} onDoubleClick={(f) => { playPixelSound.click(); setSelectedFoodForModal(f); }} />
                        ))}
                    </div>
                </SortableContext>
            </div>
    ...
    return (
        <div className="max-w-6xl mx-auto pb-20 font-mono text-slate-800">
            <div className="flex flex-col sm:flex-row gap-4 mb-8 bg-white p-4 border-4 border-slate-800 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] relative z-10">
                ...
                <button onClick={() => { playPixelSound.click(); setIsAddModalOpen(true); }} className="bg-yellow-300 border-4 border-slate-800 px-4 py-2 font-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:bg-yellow-200 flex items-center gap-2 active:translate-y-1 transition-all">
                    <Plus size={20} /> 新增食材
                </button>
            </div>
            ...
            {isAddModalOpen && (
                <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[60] p-4" onClick={() => setIsAddModalOpen(false)}>
                    <div className="bg-white border-8 border-slate-800 p-6 w-full max-w-sm shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] relative" onClick={e => e.stopPropagation()}>
                        <h3 className="text-xl font-black mb-4 italic">新增私房食材</h3>
                        <div className="space-y-4">
                            <input type="text" placeholder="食材名稱" className="w-full border-4 border-slate-800 p-2 font-bold outline-none" value={newFoodName} onChange={(e) => setNewFoodName(e.target.value)} />
                        ...
                        <div className="flex gap-2 mt-6">
                            <button onClick={handleAddNewFood} className="flex-1 bg-green-400 border-4 border-slate-800 py-2 font-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] active:translate-y-1">加入</button>
                            <button onClick={() => setIsAddModalOpen(false)} className="flex-1 bg-slate-200 border-4 border-slate-800 py-2 font-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] active:translate-y-1 text-slate-500">取消</button>
                        </div>
                    </div>
                </div>
            )}
